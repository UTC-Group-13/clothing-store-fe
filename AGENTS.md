...
- **Route công khai** (`/`, `/products`, `/product/:id`, `/cart`, `/checkout`, `/orders`, `/orders/:id`, `/login`, `/register`) — bọc trong `<Header>` + `<Footer>`.
...
- **Chatbot toàn cục**: `ChatBot` được mount ở layout public (ngoài `<main>`) để dùng trên mọi trang công khai.
...
- **State phiên chatbot**: `ChatBot` lưu `sessionId` + lịch sử tin nhắn trong `localStorage` key `chat-session`; gửi/xóa phiên qua `chatService`.
...
**Đối tượng service** (không dùng class): `productService`, `cartService`, `orderService`, `addressService`, `shippingService`, `reviewService`, `chatService`, `adminOrderService`, `adminProductService`, `adminProductFullService`, `fileUploadService`, v.v. Khi thêm service mới, tuân theo mẫu sau:
```ts
...
Toàn bộ kiểu TypeScript dùng chung nằm trong `src/types/index.ts`. Các kiểu chính: `Product`, `ProductVariant`, `VariantStock`, `CartSummary`, `CartItemDetail`, `OrderDetail`, `OrderRequest`, `AddressDTO`, `ReviewResponse`, `ReviewSummary`, `ChatMessageResponse`, `ProductFullRequest`, `ProductDetailResponse`, `FileUploadResponse`. Phân trang sử dụng `PageResponse<T>` (`content`, `pageNumber`, `totalPages`, v.v.).
...
- **Type-only import**: Dùng `import type { ... }` cho kiểu TypeScript (do `verbatimModuleSyntax: true` trong `tsconfig.app.json`).
...
| `src/components/chat/` | Widget chatbot AI nổi toàn cục ở nhóm route công khai |
